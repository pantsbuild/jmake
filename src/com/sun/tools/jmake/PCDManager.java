...
public class PCDManager {
  ...
  private Hashtable pcd;   // Maps project class names to PCDEntries
  ...
  private HashSet updatedClasses;       // This set is emptied on every new internal jmake iteration...
  private HashSet allUpdatedClasses;    // whereas in this one the names of all updated classes found during this jmake invocation are stored.
  private HashSet updatedAndCheckedClasses;
  private HashSet deletedClasses;
  private HashSet updatedJarFiles, stableJarFiles, newJarFiles, deletedJarFiles;
  ...
  public PCDManager(PCDContainer pcdc,
    ...
    newJavaFiles = new ArrayList();
    updatedJavaFiles = new HashSet();
    recompiledJavaFiles = new HashSet();
    updatedAndCheckedClasses = new HashSet();
    deletedClasses = new HashSet();
    allUpdatedClasses = new HashSet();

    updatedJarFiles = new HashSet();
    stableJarFiles = new HashSet();
    newJarFiles = new HashSet();
    deletedJarFiles = new HashSet();
  ...
  public ClassInfo getClassInfoForName(int verCode, String className) {
    PCDEntry pcde = (PCDEntry) pcd.get(className);
    if (pcde != null) return getClassInfoForPCDEntry(verCode, pcde);
    else return null;
  }


  public boolean isProjectClass(int verCode, String className) {
    if (verCode == ClassInfo.VER_OLD) return pcd.containsKey(className);
    else {
      PCDEntry pcde = (PCDEntry) pcd.get(className);
  ...
  public ClassInfo getClassInfoForPCDEntry(int verCode, PCDEntry pcde) {
    if (verCode == ClassInfo.VER_OLD) return pcde.oldClassInfo;
    ...
    if (res == null) {
      ...
      if (pcde.javaFileFullPath.endsWith(".java")) {
        ...
        if (classFile == null) return null;  // Class file not found.
	...
	try {
      ...
      res = new ClassInfo(classFileBytes, verCode, this, classFileFullPath);
  ...
  public String classAlreadyRecompiledOrUncompileable(String className) {
    ...
    if (pcde == null) {
      //!!!
      Enumeration e = pcd.keys();
      while (e.hasMoreElements()) {
	String keyName = (String) e.nextElement();
	PCDEntry entry = (PCDEntry) pcd.get(keyName);
	if (entry.className == className) System.out.println("ERROR: inconsistent entry: key = " + keyName + ", name in entry = " + entry.className);
      }
      //!!!
      throw internalException(className + " not in project when it should be");
    }
    if (pcde.checkResult == PCDEntry.CV_DELETED) return "";
    if (pcde.javaFileFullPath.endsWith(".jar")) return pcde.javaFileFullPath;
    else return (recompiledJavaFiles.contains(pcde.javaFileFullPath) ? "" : null);
  }
  ...
  public void initializeCompiler(String jcExecApp,
    ...
    if (jcMainClass == null) jcMainClass = "com.sun.tools.javac.Main";
    if (jcMethod == null) jcMethod = "compile";
  ...
  public void run() {
    ...
    while (iterNo == 0 || updatedJavaFiles.size() != 0 || newJavaFiles.size() != 0) {
      ...
      if (updatedJavaFiles.size() > 0 || newJavaFiles.size() > 0) {
        ...
        if (intermediateRes != 0) res = intermediateRes;
      }
    ...
    if (res != 0) throw compilerInteractionException("compilation error(s)", null, res);
  }
  ...
  public String[] getAllUpdatedClassesAsStringArray() {
    ...
    int i = 0;
    for (Iterator iter = allUpdatedClasses.iterator(); iter.hasNext(); ) {
      res[i++] = ((String) iter.next()).replace('/','.');
    }
    return res;
  }
  ...
  private void synchronizeProjectFilesAndPCD() {
    if (projectJavaAndJarFilesArray != null) {
      HashSet pcdJavaFilesSet = new HashSet(pcd.size()* 3 / 2);
      Enumeration pcdEntries = entriesEnum();
      while (pcdEntries.hasMoreElements()) pcdJavaFilesSet.add(((PCDEntry) pcdEntries.nextElement()).javaFileFullPath);

      HashSet canonicalPJF = new HashSet(projectJavaAndJarFilesArray.length * 3 / 2);
      ...
      for (int i = 0; i < projectJavaAndJarFilesArray.length; i++) {
        String projFileName = (String) projectJavaAndJarFilesArray[i];
        ...
        if (projFile == null) throw new PrivateException(new FileNotFoundException("specified source file " + projFileName + " not found."));
	...
	if (backSlashFileSeparator) absoluteProjFileName = Utils.convertDriveLetterToLowerCase(absoluteProjFileName);
        canonicalPJF.add(absoluteProjFileName);
        if (! pcdJavaFilesSet.contains(absoluteProjFileName)) {
	  if (absoluteProjFileName.endsWith(".java")) newJavaFiles.add(absoluteProjFileName);
	  else if (absoluteProjFileName.endsWith(".jar")) newJarFiles.add(absoluteProjFileName);
	  else throw new PrivateException(new PublicExceptions.InvalidSourceFileExtensionException("specified source file " + projFileName + " has an invalid extension (not .java or .jar)."));
        }
      }
      ...
      for (Enumeration keys = pcd.keys(); keys.hasMoreElements(); ) {
        ...
        PCDEntry e = (PCDEntry) pcd.get(key);
        e.oldClassInfo.restorePCDM(this);
        if (! canonicalPJF.contains(e.javaFileFullPath)) {
	  deletedClasses.add(key);
	  if (e.javaFileFullPath.endsWith(".jar")) deletedJarFiles.add(e.javaFileFullPath);
	  ...
	  if (fileName.endsWith(".java")) newJavaFiles.add(fileName);
	  else if (fileName.endsWith(".jar")) newJarFiles.add(fileName);
	  else throw new PrivateException(new PublicExceptions.InvalidSourceFileExtensionException("specified source file " + fileName + " has an invalid extension (not .java or .jar)."));
        }
      }
      
      HashSet removedJavaAndJarFilesSet = null;
      if (removedJavaAndJarFilesArray != null) {
        removedJavaAndJarFilesSet = new HashSet();
	  ...
	  if (fileName.endsWith(".jar")) deletedJarFiles.add(fileName);
	}
      }
      ...
      for (Enumeration keys = pcd.keys(); keys.hasMoreElements(); ) {
        ...
        PCDEntry e = (PCDEntry) pcd.get(key);
        e.oldClassInfo.restorePCDM(this);
        if (removedJavaAndJarFilesSet != null && removedJavaAndJarFilesSet.contains(e.javaFileFullPath)) {
	  deletedClasses.add(key);
  ...
  private void updateClassFilesInfoInPCD(int compilationResult) {
    ...
    if (compilationResult != 0) {
      for (Iterator iter = updatedAndCheckedClasses.iterator(); iter.hasNext(); ) {
        PCDEntry entry = (PCDEntry) pcd.get((String) iter.next());
	if (entry.checkResult == PCDEntry.CV_DELETED && entry.oldClassInfo.directlyEnclosingClass != "") {
	  PCDEntry enclEntry = (PCDEntry) pcd.get(entry.oldClassInfo.directlyEnclosingClass);
    ...
    for (Iterator iter = updatedAndCheckedClasses.iterator(); iter.hasNext(); ) {
      ...
      if (entry.checkResult == PCDEntry.CV_UNCHECKED) continue;
      if (entry.checkResult == PCDEntry.CV_DELETED) {
        if (compilationResult == 0) pcd.remove(className);  // Only if consistency checking is ok, a deleted class can be safely removed from the PCD
        ...
        if (entry.newClassInfo == null) {  // "Safety net" for the (hopefully unlikely) case we overlooked something before...
          ...
          if (entry.checkResult == PCDEntry.CV_NEW) pcd.remove(className);
          else continue;
        }
  ...
  private void findUpdatedJavaAndJarFiles() {
    boolean projectSpecifiedAsAllSources = projectJavaAndJarFilesArray != null;
    for (Enumeration entries = entriesEnum(); entries.hasMoreElements(); ) {
      PCDEntry entry = (PCDEntry) entries.nextElement();
      if (deletedClasses.contains(entry.className)) continue;
	...
	initializeClassFileFullPath(entry);
        if (projectSpecifiedAsAllSources) {
          ...
          if (classFileObsoleteOrDeleted(entry)) updatedJavaFiles.add(entry.javaFileFullPath);
	...
	entry.checked = true;
        ...
        if (projectJavaAndJarFilesArray != null) entry.checked = ! checkJarFileForUpdate(entry);
      }
    }
	...
	if (updatedJavaAndJarFilesArray[i].endsWith(".java")) updatedJavaFiles.add(updatedJavaAndJarFilesArray[i]);
	else updatedJarFiles.add(updatedJavaAndJarFilesArray[i]);
      }
    }
  }
  ...
  private boolean classFileObsoleteOrDeleted(PCDEntry entry) {
    ...
    if (classFile == null || ! classFile.exists()) return true; // Class file has been deleted
    File javaFile = new File(entry.javaFileFullPath); // Guaranteed to exist at this point
    if (classFile.lastModified() < javaFile.lastModified()) return true;      
    return false;
  }
  ...
  private boolean checkJarFileForUpdate(PCDEntry entry) {
    ...
    if (stableJarFiles.contains(jarFileName)) return false;
    else if (updatedJarFiles.contains(jarFileName) || newJarFiles.contains(jarFileName) ||
             deletedJarFiles.contains(jarFileName)) return true;
    else {
  ...
  public int recompileUpdatedJavaFiles() {
    if (externalApp != null) return recompileUpdatedJavaFilesUsingExternalMethod();
    else return recompileUpdatedJavaFilesOurselves();
  }
  ...
  private int recompileUpdatedJavaFilesOurselves() {
    ...
    if ((compilerBootClassPath = ClassPath.getCompilerBootClassPath()) != null) argsNo += 2;
    if ((compilerExtDirs = ClassPath.getCompilerExtDirs()) != null) argsNo += 2;
    if (jcExecApp != null) argsNo++;
    String args[] = new String[argsNo];
    int pos = 0;
    if (jcExecApp != null)  args[pos++] = jcExecApp;
    for (int i = 0; i < addArgsNo; i++) args[pos++] = (String) javacAddArgs.get(i);
    args[pos++] = "-classpath";
  ...
  private int recompileUpdatedJavaFilesUsingExternalMethod() {
    ...
    int i = 0;
    for (Iterator iter = updatedJavaFiles.iterator(); iter.hasNext(); i++) recompiledJavaFiles.add(fileNames[i] = (String) iter.next());
    for (int j = 0; j < newJavaFiles.size(); j++) recompiledJavaFiles.add(fileNames[i++] = (String) newJavaFiles.get(j));
      
    try {
      Integer res = (Integer) externalCompileSourceFilesMethod.invoke(externalApp, new Object[]{fileNames});
      return res.intValue();
  ...
  private void findClassFilesForNewJavaAndJarFiles() {
    for (int i = 0; i < newJavaFiles.size(); i++) {
      String javaFileFullPath = (String) newJavaFiles.get(i);
      PCDEntry pcde = findClassFileOnFilesystem(javaFileFullPath, null, null);
      if (pcde == null) continue;                 // .class file not found - compilation error
      ...
      } else findAndUpdateAllNestedClassesForClass(pcde, true);
    }
    
    for (Iterator iter = newJarFiles.iterator(); iter.hasNext(); ) {
      String newJarFile = (String) iter.next();
  ...
  private PCDEntry findClassFileOnFilesystem(String javaFileFullPath, PCDEntry enclosingClassPCDE, String nestedClassFullName) {
    ...
    if (backSlashFileSeparator) fullClassName = fullClassName.replace(File.separatorChar, '/');
      
    byte classFileBytes[] = Utils.readFileIntoBuffer(classFile);
    ClassInfo classInfo = new ClassInfo(classFileBytes, ClassInfo.VER_NEW, this, classFileFullPath);
    ...
    } else fullClassName = classInfo.name;
 
    if (enclosingClassPCDE != null) javaFileFullPath = enclosingClassPCDE.javaFileFullPath;
    ...
    if (pcd.containsKey(fullClassName)) {
      ...
      if (pcde.checked) throw new PrivateException(new PublicExceptions.DoubleEntryException(
  ...
  private void findAndUpdateAllNestedClassesForClass(PCDEntry pcde, boolean move) {
    ...
    if (classInfo.nestedClasses == null) return;
    ...
    for (int i = 0; i < nestedClasses.length; i++) {
      ...
      if (nestedPCDE == null) {
        if (isJavaSourceFile) nestedPCDE = findClassFileOnFilesystem(null, pcde, nestedClasses[i]);
        ...
        if (nestedPCDE == null) continue;  // Probably a compilation error, such that enclosing class is compiled but nested is not.
      }
      if (move) {
        if (deletedClasses.contains(nestedClasses[i])) deletedClasses.remove(nestedClasses[i]);
      	nestedPCDE.javaFileFullPath = javaFileFullPath;
        if (javaFileFullPath.endsWith(".java")) {
  	  nestedPCDE.classFileFullPath = Utils.getClassFileFullPathForNestedClass(enclosingClassFileFullPath, nestedClasses[i]);
        } else nestedPCDE.classFileFullPath = javaFileFullPath;
      }
      if (nestedPCDE.newClassInfo == null) getClassInfoForPCDEntry(ClassInfo.VER_NEW, nestedPCDE);
  ...
  private void dealWithNestedClassesForUpdatedJavaFiles() {
    if (updatedJavaFiles.size() == 0) return;
    ...
    for (Enumeration entries = entriesEnum(); entries.hasMoreElements(); ) {
      ...
      if (pcde.checkResult == PCDEntry.CV_NEW) continue;  // This class has just been added to the PCD
      if (updatedJavaFiles.contains(pcde.javaFileFullPath)) {
        ClassInfo oldClassInfo = pcde.oldClassInfo;
        ClassInfo newClassInfo = getClassInfoForPCDEntry(ClassInfo.VER_NEW, pcde);
        if (newClassInfo == null) continue; // Class file deleted then not re-created due to a compilation error somewhere.
        if (oldClassInfo.nestedClasses != null || newClassInfo.nestedClasses != null) updatedEntries.add(pcde);
      }
    }
  ...
  private void dealWithNestedClassesForUpdatedPCDEntries(ArrayList entries, boolean move) {
    for (int i = 0; i < entries.size(); i++) {
      PCDEntry pcde = (PCDEntry) entries.get(i);
      ClassInfo oldClassInfo = pcde.oldClassInfo;
      ...
      if (newClassInfo.nestedClasses != null) {
        ...
        if (oldClassInfo.nestedClasses != null) {  // Check if any old nested classes don't exist anymore
          for (int j = 0; j < oldClassInfo.nestedClasses.length; j++) {
            ...
            if (! found) deletedClasses.add(oldNestedClass);
          }
        }
      } else {  // newNestedClasses == null and oldNestedClasses != null, so all nested classes have been removed in the new version
        for (int j = 0; j < oldClassInfo.nestedClasses.length; j++) deletedClasses.add(oldClassInfo.nestedClasses[j]);
      }
    }
  }
  ...
  private void findUpdatedClasses() {
    ...
    while (entries.hasMoreElements()) {
      ...
      if (updatedAndCheckedClasses.contains(className) || deletedClasses.contains(className)) continue;
      if (! entry.javaFileFullPath.endsWith(".java")) continue; // classes from (updated) .jars have been dealt with separately
  ...
  private void checkUpdatedClasses() {
    for (Iterator iter = updatedClasses.iterator(); iter.hasNext(); ) {
      String className = (String) iter.next();
      PCDEntry pcde = (PCDEntry) pcd.get(className);
      ...
      if (pcde.oldClassInfo.directlyEnclosingClass != "") {
        ...
        ClassInfo enclosingClassInfo = getClassInfoForName(ClassInfo.VER_NEW, pcde.oldClassInfo.directlyEnclosingClass);
	...
	if (enclosingClassInfo.nestedClasses != null) {  // Can be that this nested class was the only one for enclosing class, and it's deleted now
      ...
      if (! (pcde.oldClassInfo.isNonMemberNestedClass && pcde.newClassInfo.isNonMemberNestedClass)) {
        ...
        pcde.checkResult = cv.compareClassVersions(pcde) ? PCDEntry.CV_COMPATIBLE : PCDEntry.CV_INCOMPATIBLE;
        String affectedClasses[] = cv.getAffectedClasses();
        if (affectedClasses != null) {
          for (int i = 0; i < affectedClasses.length; i++) {
            PCDEntry affEntry = (PCDEntry) pcd.get(affectedClasses[i]);
  ...
  private void checkDeletedClasses() {
    for (Iterator iter = deletedClasses.iterator(); iter.hasNext(); ) {
      String className = (String) iter.next();
      PCDEntry pcde = (PCDEntry) pcd.get(className);
      ...
      if (! oldCI.isNonMemberNestedClass) { // See the comment above
        ...
        if (affectedClasses != null) {
          for (int i = 0; i < affectedClasses.length; i++) {
            PCDEntry affEntry = (PCDEntry) pcd.get(affectedClasses[i]);
            if (deletedClasses.contains(affEntry.className)) continue;
            updatedJavaFiles.add(affEntry.javaFileFullPath);
          }
        }
      }
  ...
  private void processAllClassesFromJarFile(String jarFileName) {
    ...
    ArrayList updatedEntries = new ArrayList();
    ArrayList movedEntries = new ArrayList();
	...
	if (pcde.oldClassFileFingerprint != classFileFP) {  // This class has been updated
	  ...
	  pcde.newClassInfo = new ClassInfo(classFileBytes, ClassInfo.VER_NEW, this, fullClassName);
          if (pcde.oldClassInfo.nestedClasses != null || pcde.newClassInfo.nestedClasses != null) updatedEntries.add(pcde);
	} else pcde.oldClassFileLastModified = jarFileLastMod;
	...
	ClassInfo classInfo = new ClassInfo(classFileBytes, ClassInfo.VER_NEW, this, fullClassName);
        ...
        updatedAndCheckedClasses.add(fullClassName); // So that the above happens
        pcd.put(fullClassName, pcde);
        if (pcde.newClassInfo.nestedClasses != null) newEntries.add(pcde);
      }
    }
    
    dealWithNestedClassesForUpdatedPCDEntries(updatedEntries, false);
    dealWithNestedClassesForUpdatedPCDEntries(movedEntries, true);
    for (int i = 0; i < newEntries.size(); i++) findAndUpdateAllNestedClassesForClass((PCDEntry) newEntries.get(i), false);
  }


  /** Determine new, deleted and updated classes coming from updated .jar files. */
  private void dealWithClassesInUpdatedJarFiles() {
    if (updatedJarFiles.size() == 0) return;
	...
	if (! pcde.checked) deletedClasses.add(pcde.className);
      }
    }
  }
  ...
  private void initializeDestDir(String inDestDir) {
    if (! (inDestDir == null || inDestDir.equals(""))) {
      ...
      if (dir == null) throw new PrivateException(new IOException("specified directory " + inDestDir + " cannot be created."));
      inDestDir = getCanonicalPath(dir);
      if (! inDestDir.endsWith(File.separator)) inDestDir += File.separatorChar;
      destDir = inDestDir;
      destDirSpecified = true;
    } else destDirSpecified = false;
  }
  ...
  private void initializeClassFileFullPath(PCDEntry entry) {
    ...
    if (destDirSpecified) {
      ...
      if (cutIndex != -1) javaFileDir = javaFileDir.substring(0, cutIndex+1);
      String classFileName = entry.className;
      cutIndex = classFileName.lastIndexOf('/');
      if (cutIndex != -1) classFileName = classFileName.substring(cutIndex+1); 
      classFileFullPath = javaFileDir + classFileName + ".class";
    }
    if (backSlashFileSeparator) classFileFullPath = classFileFullPath.replace('/', File.separatorChar);
