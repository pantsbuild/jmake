...
public class ClassFileReader extends BinaryFileReader {
  ...
  private void readPreamble() {
    int magic = nextInt();
    if (magic != JAVA_MAGIC) throw classFileParseException("Illegal start of class file");
    ...
    if (majorVersion > JDK14_MAJOR_VERSION ||
      ...
      if (majorVersion == JDK15_MAJOR_VERSION) classInfo.javacTargetRelease = Utils.JAVAC_TARGET_RELEASE_15;
      else throw classFileParseException("Wrong version: " + majorVersion + "." + minorVersion);
    } else classInfo.javacTargetRelease = Utils.JAVAC_TARGET_RELEASE_OLDEST;
  }
  ...
  private void readConstantPool(boolean readFullInfo) {
    ...
    for (i = 0; i < cpOffsets.length; i++) {
      ...
      switch (cpTags[i]) {
        case CONSTANT_Class:
          utf8Idx = getChar(ofs);
          classInfo.cpoolRefsToClasses[curClassRef++] = classNameAtCPIndex(utf8Idx, classInfo.isRefClassArray, curClassRef-1);
	  //System.out.println("Read cpool ref to class: " + classInfo.cpoolRefsToClasses[curClassRef-1]);
        ...
        case CONSTANT_Fieldref:
          ...
          if (cpTags[classIdx] != CONSTANT_Class || cpTags[nameAndTypeIdx] != CONSTANT_NameandType) badCPReference(ofs, i);
          classInfo.cpoolRefsToFieldClasses[curFieldRef] = classNameAtCPIndex(getChar(cpOffsets[classIdx]));

          ofs = cpOffsets[nameAndTypeIdx];
          nameIdx = getChar(ofs);
          sigIdx = getChar(ofs+2);
          if (cpTags[nameIdx] != CONSTANT_Utf8 || cpTags[sigIdx] != CONSTANT_Utf8) badCPReference(ofs, i);
          classInfo.cpoolRefsToFieldNames[curFieldRef] = utf8AtCPIndex(nameIdx);
          classInfo.cpoolRefsToFieldSignatures[curFieldRef] = signatureAtCPIndex(sigIdx);
	  ...
	  //                   classInfo.cpoolRefsToFieldSignatures[curFieldRef]);
        ...
        case CONSTANT_InterfaceMethodref:
          ...
          nameAndTypeIdx = getChar(ofs+2);
          if (cpTags[classIdx] != CONSTANT_Class || cpTags[nameAndTypeIdx] != CONSTANT_NameandType) badCPReference(ofs, i);
          classInfo.cpoolRefsToMethodClasses[curMethodRef] = classNameAtCPIndex(getChar(cpOffsets[classIdx]));
          
          ofs = cpOffsets[nameAndTypeIdx];
          nameIdx = getChar(ofs);
          sigIdx = getChar(ofs+2);
          if (cpTags[nameIdx] != CONSTANT_Utf8 || cpTags[sigIdx] != CONSTANT_Utf8) badCPReference(ofs, i);
          classInfo.cpoolRefsToMethodNames[curMethodRef] = utf8AtCPIndex(nameIdx);
          classInfo.cpoolRefsToMethodSignatures[curMethodRef] = signatureAtCPIndex(sigIdx);
  ...
  private void readIntermediate() {
    ...
    classIdx = nextChar();
    if (cpTags[classIdx] != CONSTANT_Class) throw classFileParseException("Bad reference to this class name");
    classInfo.name = classNameAtCPIndex(getChar(cpOffsets[classIdx]));
    superClassIdx = nextChar();
    if (classInfo.name != "java/lang/Object") {
      if (cpTags[superClassIdx] != CONSTANT_Class) throw classFileParseException("Bad reference to super class name");
    ...
    if (intfCount != 0) {
      ...
      for (i = 0; i < intfCount; i++) {
        classIdx = nextChar();
        if (cpTags[classIdx] != CONSTANT_Class) throw classFileParseException("Bad reference to an implemented interface");
  ...
  private void readFields() {
    ...
    for (i = 0; i < definedFieldCount; i++) {
      ...
      boolean recordField = ! (Modifier.isPrivate(flags) &&
      ...
      for (j = 0; j < attrCount; j++) {
        ...
        if (recordField && utf8AtCPIndex(attrNameIdx).equals("ConstantValue") &&
          ...
          if (classInfo.primitiveConstantInitValues == null)
            classInfo.primitiveConstantInitValues = new Object[definedFieldCount];
          int constValueIdx = nextChar();
          switch (cpTags[constValueIdx]) {
            case CONSTANT_String :
              classInfo.primitiveConstantInitValues[ri] = utf8AtCPIndex(getChar(cpOffsets[constValueIdx]));
              break;

            case CONSTANT_Integer :
              classInfo.primitiveConstantInitValues[ri] = new Integer(getInt(cpOffsets[constValueIdx]));
              break;

            case CONSTANT_Long :
              classInfo.primitiveConstantInitValues[ri] = new Long(getLong(cpOffsets[constValueIdx]));
              break;

            case CONSTANT_Float :
              classInfo.primitiveConstantInitValues[ri] = new Float(getFloat(cpOffsets[constValueIdx]));
              break;

            case CONSTANT_Double :
              classInfo.primitiveConstantInitValues[ri] = new Double(getDouble(cpOffsets[constValueIdx]));
              break;
    ...
    for (i = 0; i < methodCount; i++) {
      ...
      for (j = 0; j < attrCount; j++) {
        ...
        if (utf8AtCPIndex(attrNameIdx).equals("Exceptions")) {
          if (classInfo.checkedExceptions == null) classInfo.checkedExceptions = new String[methodCount][];
          int nExceptions = nextChar();
          String exceptions[] = new String[nExceptions];
          for (int k = 0; k < nExceptions; k++) {
            int excClassIdx = nextChar();
            if (cpTags[excClassIdx] != CONSTANT_Class) badCPEntry(excClassIdx);
            exceptions[k] = classNameAtCPIndex(getChar(cpOffsets[excClassIdx]));
          }
          ...
          if (innerClassNameIdx != 0) {  // Nested class is not anonymous
            ...
            if (! nestedClassFullName.equals(classInfo.name + "$" + nestedClassSimpleName)) {
              ...
              if (classInfo.javacTargetRelease == Utils.JAVAC_TARGET_RELEASE_OLDEST) {
                ...
                if (! nestedClassFullName.equals(classInfo.name + "$" + count + "$" + nestedClassSimpleName)) continue;
                else nonMemberClassCount = count;
              } else { // JDK 1.5
                if (nestedClassFullName.startsWith(classInfo.name + "$") && nestedClassFullName.endsWith(nestedClassSimpleName)) {
                  ...
                  int endIdx = nestedClassFullName.length() - nestedClassSimpleName.length();
                  for (; k < endIdx; k++) {
                    if (! Character.isDigit(nestedClassFullName.charAt(k))) break;
                  }
                  if (k < endIdx) continue;
                } else continue;
              }
            }
          } else {                       // Nested class is anonymous
            if (! nestedClassFullName.equals(classInfo.name + "$" + nonMemberClassCount)) continue;
	    else nonMemberClassCount++;
          }
  ...
  private String utf8AtCPIndex(int idx) {
    if (cpTags[idx] != CONSTANT_Utf8) throw classFileParseException("Constant pool entry " + idx + " should be UTF8 constant");
    if (cpObjectCache[idx] == null) {
      ...
      cpObjectCache[idx] = (new String(buf, cpOffsets[idx]+2, utf8Len)).intern();
    }
  ...
  private String classNameAtCPIndex(int idx, boolean isRefClassArray[], int isArrayIdx) {
    ...
    if (cpObjectCache[idx] == null) {
      ...
      if (isRefClassArray != null) isRefClassArray[isArrayIdx] = isArray;
    }
    return (String) cpObjectCache[idx];
  }
