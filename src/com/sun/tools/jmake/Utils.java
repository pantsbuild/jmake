...
public class Utils {
  ...
  public static String getPackageName(String clazzName) {
    ...
    if (ldi == -1) return "";
    else return clazzName.substring(0, ldi).intern();
  }
  ...
  public static String getDirectlyEnclosingClass(String clazzName, int javacTargetRelease) {
    ...
    if (javacTargetRelease == JAVAC_TARGET_RELEASE_15) {
      ...
      if (lldi == -1 || !Character.isDigit(clazzName.charAt(lldi+1))) return clazzName.substring(0, ldi).intern();
      else return clazzName.substring(0,lldi).intern();
    }
  }
  ...
  public static String getClassFileFullPathForNestedClass(String enclosingClassFileFullPath, String nestedClassFullName) {
    ...
    if (cutIndex < 0) nestedClassLocalName = nestedClassFullName;
    else nestedClassLocalName = nestedClassFullName.substring(cutIndex+1);
  ...
  public static File checkOrCreateDirForName(String name) {
    ...
    if (file.exists()) {
      if (! file.isDirectory()) throw new PrivateException(new PublicExceptions.InternalException(file + " is not a directory."));
      return file;
    }
    return null;
  }
  ...
  public static void printInfoMessage(String message) {
    if (printInfoMessages) out.println(message);
  }
  
  public static void printInfoMessageNoEOL(String message) {
    if (printInfoMessages) out.print(message);
  }
  
  public static void printWarningMessage(String message) {
    if (! printWarningMessages) return;
    if (warningNo < warningLimit) warn.println(message);
    else if (warningNo == warningLimit) warn.println("jmake: more than " + warningLimit + " warnings.");
    warningNo++;
  }
  
  public static void printErrorMessage(String message) {
    if (printErrorMessages) err.println("jmake: " + message);
  }
  ...
  public static void setTimingOn() { timingOn = true; }
  ...
  public static void printTiming(String message, int slot) {
    if (timingOn) printInfoMessage("========== " + message + " time = " + timings[slot]);
  }
