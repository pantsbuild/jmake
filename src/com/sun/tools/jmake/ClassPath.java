...
public class ClassPath {
  ...
  public static void resetOnFinish() {
    projectClassPath = standardClassPath = bootClassPath = extClassPath = null;
    compilerUserClassPath = null;
    standardClassPathStr = projectClassPathStr = bootClassPathStr = extDirsStr = null;
  ...
  public static void setExtDirs(String value) throws PublicExceptions.InvalidCmdOptionException {
    ...
    for (StringTokenizer tok = new StringTokenizer(value, File.pathSeparator); tok.hasMoreTokens(); ) {
      ...
      for (int i = 0; i < extJars.length; i++) extClassPathElements.add(extDir + File.separator + extJars[i]);
    }
  ...
  public static void initializeAllClassPaths() {
    ...
    if (standardClassPathStr == null && projectClassPathStr == null) compilerUserClassPath = ".";
    else if (standardClassPathStr == null) compilerUserClassPath = projectClassPathStr;
    else if (projectClassPathStr == null) compilerUserClassPath = standardClassPathStr;
    else compilerUserClassPath = standardClassPathStr + File.pathSeparator + projectClassPathStr;
    ...
    if (projectClassPathStr == null) projectClassPath = new ClassPath();
  ...
  public static String getCompilerUserClassPath() { return compilerUserClassPath; }

  /** Will return null if boot class path wasn't explicitly specified */
  public static String getCompilerBootClassPath() { return bootClassPathStr; }
  
  /** Will return null if extdirs weren't explicitly specified */
  public static String getCompilerExtDirs()       { return extDirsStr; }
  ...
  public static void addAllImplementedInterfaceNames(String className, Set intfSet, PCDManager pcdm) {
    if (className == "java/lang/Object") return;
    ClassInfo ci = getClassInfoForName(className, pcdm);
    if (ci == null) return;
    ...
    if (superName != null) addAllImplementedInterfaceNames(superName, intfSet, pcdm);
  }


  public static String[] getProjectJars() {
    if (projectClassPath == null || projectClassPath.isEmpty()) return null;
    PathEntry paths[] = projectClassPath.paths;
    String[] ret = new String[paths.length];
    for (int i = 0; i < paths.length; i++) ret[i] = paths[i].toString();
    return ret;
  }
  ...
  public static ClassInfo getClassInfoForName(String className, PCDManager pcdm) {
    ...
    if (buf == null) buf = extClassPath.getBytesForClass(className);
    if (buf == null) buf = standardClassPath.getBytesForClass(className);
    if (buf == null) buf = projectClassPath.getBytesForClass(className);
  ...
  public static ClassLoader getClassLoaderForPath(String classPath) throws Exception {
    ...
    for (int i = 0; i < paths.length; i++) {
      ...
      if (! (dirOrJar.startsWith("file://") || dirOrJar.startsWith("http://"))) {
        ...
        if (isWindows && dirOrJar.charAt(1) == ':') dirOrJar = "/" + dirOrJar;
  ...
  private ClassPath(String classPath, boolean isJarOnly) throws PublicExceptions.InvalidCmdOptionException {
    if (classPath == null) throw new PublicExceptions.InvalidCmdOptionException("null argument");
  ...
  private void init(ArrayList pathEntries, boolean isJarOnly) throws PublicExceptions.InvalidCmdOptionException {
    if (pathEntries == null) throw new PublicExceptions.InvalidCmdOptionException("null argument");
	...
	try {
	  if (file.exists() && file.canRead()) {
	    if (file.isDirectory()) {
	      if (isJarOnly) throw new PublicExceptions.InvalidCmdOptionException("directories are not allowed on this class path: " + path);
	      vec.add(new Dir(file));
	    } else vec.add(new Zip(new ZipFile(file)));
	  } else if (isJarOnly) throw new IOException("file does not exist");
	} catch(IOException e) {
	  if (isJarOnly) throw new PublicExceptions.InvalidCmdOptionException("error initializing class path component " + path + ": " + e.getMessage());
	}
      }
    }
  ...
  public String toString() {
    ...
    String res = "";
    for (int i = 0; i < paths.length; i++) res += paths[i].toString();
    return res;
  }
  ...
  private static class Dir extends PathEntry {
    ...
    byte[] getBytesForClassFile(String fileName) {
      ...
      if (file.exists()) return Utils.readFileIntoBuffer(file);
      else return null;
    }
    
    public String toString() { return dir.toString(); }
  }
  ...
  private static class Zip extends PathEntry {
    ...
    byte[] getBytesForClassFile(String fileName) {
      ...
      if (entry != null) return Utils.readZipEntryIntoBuffer(zip, entry);
      else return null;
    }
