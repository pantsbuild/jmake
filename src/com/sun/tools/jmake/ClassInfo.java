...
public class ClassInfo {
  ...
  public ClassInfo(byte[] classFileBytes, int verCode, PCDManager pcdm, String classFileFullPath) {
    ...
    directlyEnclosingClass = Utils.getDirectlyEnclosingClass(name, this.javacTargetRelease);
  ...
  public boolean isInterface() { return Modifier.isInterface(accessFlags); }
  public boolean isAbstract()  { return Modifier.isAbstract(accessFlags); }
  public boolean isPublic()    { return Modifier.isPublic(accessFlags); }
  ...
  private void addImplementedInterfaceNames(boolean directOnly, Set intfSet) {    
	...
	ClassInfo superIntfInfo = pcdm.getClassInfoForName(verCode, superIntfName);
	if (superIntfInfo == null) {  // Class not in project
	  ClassPath.addAllImplementedInterfaceNames(superIntfName, intfSet, pcdm);
  	} else superIntfInfo.addImplementedInterfaceNames(false, intfSet);
      }
    }
    
    if (directOnly || superName == null || superName == "java/lang/Object") return;
    ...
    } else superInfo.addImplementedInterfaceNames(false, intfSet);
  }
  ...
  public ClassInfo[] getDirectSubclasses() {    
    if (directSubclasses != null) return directSubclasses;
    ...
    while (entries.hasMoreElements()) {
      ...
      if (classInfo == null) continue;  // New or deleted class, depending on verCode
      if (classInfo.superName == name) listRes.add(classInfo);
    }
    
    directSubclasses = (ClassInfo[]) listRes.toArray(new ClassInfo[listRes.size()]);
  ...
  public static boolean constFieldInitValuesEqual(ClassInfo oldClassInfo, int oldFieldNo,
    ...
    Object oldInitValue = oldClassInfo.primitiveConstantInitValues == null ?
           null : oldClassInfo.primitiveConstantInitValues[oldFieldNo];
    Object newInitValue = newClassInfo.primitiveConstantInitValues == null ?
           null : newClassInfo.primitiveConstantInitValues[newFieldNo];
    if (oldInitValue == newInitValue) return true;
    if (oldInitValue == null || newInitValue == null) return false;
           
    if (oldInitValue instanceof Integer) {
      if (((Integer) oldInitValue).intValue() == ((Integer) newInitValue).intValue()) return true;
      else return false;
    } else if (oldInitValue instanceof String) {
      if (((String) oldInitValue) == ((String) newInitValue)) return true;
      else return false;
    } else if (oldInitValue instanceof Long) {
      if (((Long) oldInitValue).longValue() == ((Long) newInitValue).longValue()) return true;
      else return false;
    } else if (oldInitValue instanceof Float) {
      if (((Float) oldInitValue).floatValue() == ((Float) newInitValue).floatValue()) return true;
      else return false;
    } else if (oldInitValue instanceof Double) {
      if (((Double) oldInitValue).doubleValue() == ((Double) newInitValue).doubleValue()) return true;
      else return false;
    }
  ...
  public boolean implementsInterfaceDirectly(String intfName) {
    ...
    for (int i = 0; i < interfaces.length; i++) {
      if (intfName == interfaces[i]) return true;
    }
    return false;
  }
  ...
  public boolean implementsIntfOrSubintfDirectly(String intfName) {
    ...
    for (int i = 0; i < interfaces.length; i++) {
      ...
      ClassInfo superIntfInfo = pcdm.getClassInfoForName(verCode, interfaces[i]);
      if (superIntfInfo == null) continue;  // Class not in project
      if (superIntfInfo.implementsIntfOrSubintfDirectly(intfName)) return true;
    }
    return false;
  }
  ...
  public boolean implementsInterfaceDirectlyOrIndirectly(String intfName) {
    ...
    if (implementsIntfOrSubintfDirectly(intfName)) return true;

    if (superName != null) {
      ClassInfo superInfo = pcdm.getClassInfoForName(verCode, superName);
      if (superInfo == null) return false;  // Class not in project
  ...
  public boolean declaresField(ClassInfo classInfo, int fieldNo) {
    ...
    for (int i = 0; i < fieldNames.length; i++) {
      if (fieldName == fieldNames[i] && fieldSignature == fieldSignatures[i]) return true;
    }
    return false;
  }
  ...
  public boolean declaresField(String name, String signature, boolean isStatic) {
    ...
    for (int i = 0; i < fieldNames.length; i++) {
      if (name == fieldNames[i] && signature == fieldSignatures[i] && Modifier.isStatic(fieldAccessFlags[i]) == isStatic) return true;
    }
    return false;
  }
  ...
  public boolean declaresMethod(ClassInfo classInfo, int methodNo) {
    ...
    for (int i = 0; i < methodNames.length; i++) {
      if (methodName == methodNames[i] && methodSignature == methodSignatures[i]) return true;
    }
    return false;
  }
  ...
  public int getDeclaredMethodPos(ClassInfo classInfo, int methodNo) {
    ...
    for (int i = 0; i < methodNames.length; i++) {
      if (methodName == methodNames[i] && methodSignature == methodSignatures[i]) return i;
    }
    return -1;
  }
  ...
  public int declaresSameNameMethod(String methodName) {
    ...
    for (int j = 0; j < methodNames.length; j++) {
      if (methodNames[j] == methodName) return j;
    }
    return -1;
  }
  ...
  public boolean referencesClass(String className, boolean isRefTypeInterface, int thorDegree) {
    ...
    if (thorDegree == 0) {
      if (cpoolRefsToClasses == null) return false;
      for (i = 0; i < cpoolRefsToClasses.length; i++) {
        if (!isRefClassArray[i] && className == cpoolRefsToClasses[i]) return true;
      }
    } else {
      if (isSubclassOf(className, (thorDegree == 1))) return true;
      if (isRefTypeInterface) {
        if (thorDegree == 1) {
          if (implementsInterfaceDirectly(className)) return true;
        } else {
          // Check for indirectly implemented interfaces
          if (implementsInterfaceDirectlyOrIndirectly(className)) return true;
        }
      }
      
      if (cpoolRefsToClasses != null) {
        for (i = 0; i < cpoolRefsToClasses.length; i++) {
          if (className == cpoolRefsToClasses[i]) return true;
        }
      }
      if (thorDegree == 2) {
        ...
        if (cpoolRefsToFieldSignatures != null) {
          for (i = 0; i < cpoolRefsToFieldSignatures.length; i++) {
            if (signatureIncludesClassName(cpoolRefsToFieldSignatures[i], className)) return true;          
          }
        }
        if (cpoolRefsToMethodNames != null) {
          for (i = 0; i < cpoolRefsToMethodSignatures.length; i++) {
            if (signatureIncludesClassName(cpoolRefsToMethodSignatures[i], className)) return true;            
          }
        }
      }
      ...
      if (fieldSignatures != null) {
        for (i = 0; i < fieldSignatures.length; i++) {
          if (signatureIncludesClassName(fieldSignatures[i], className)) return true;            
        }
      }
      if (methodSignatures != null) {
        for (i = 0; i < methodSignatures.length; i++) {
          if (signatureIncludesClassName(methodSignatures[i], className)) return true;            
        }
      }
        ...
        for (i = 0; i < checkedExceptions.length; i++) {
          if (checkedExceptions[i] != null) {
            ...
            for (int j = 0; j < excArray.length; j++) {
              if (className == excArray[j]) return true;
            }
          }
        }
      }
  ...
  public boolean isSubclassOf(String className, boolean directOnly) {
    ...
    while (superName != null) {
      if (className == superName) return true;
      ClassInfo classInfo = pcdm.getClassInfoForName(verCode, superName);
      if (classInfo == null) break;  // Class not in project
      superName = classInfo.superName;
    }
    return false;
  }
  ...
  public boolean referencesField(ClassInfo fieldDefClassInfo, int fieldNo) {
    if (cpoolRefsToFieldNames == null) return false;
    ...
    for (int i = 0; i < cpoolRefsToFieldNames.length; i++) {
      if (fieldName == cpoolRefsToFieldNames[i] && fieldSig == cpoolRefsToFieldSignatures[i]) {
        if (fieldDefClassName == cpoolRefsToFieldClasses[i]) return true;  // "real" reference
        else {  // Check if this is a "fake" reference that resolves to the above "real" reference
          ClassInfo classInThisCpool = pcdm.getClassInfoForName(verCode, cpoolRefsToFieldClasses[i]);
          if (classInThisCpool == null) continue;  // Class not in project
          if (! classInThisCpool.isSubclassOf(fieldDefClassInfo.name, false)) continue;
  ...
  public boolean referencesMethod(ClassInfo methodDefClassInfo, int methodNo) {
    if (cpoolRefsToMethodNames == null) return false;
    ...
    for (int i = 0; i < cpoolRefsToMethodNames.length; i++) {
      if (methodName == cpoolRefsToMethodNames[i] && methodSig == cpoolRefsToMethodSignatures[i]) {
        if (methodDefClassName == cpoolRefsToMethodClasses[i]) return true;  // "real" reference
        else {  // Check if this is a "fake" reference that resolves to the above "real" reference
          // Be careful - class in the cpool may be not a project class (e.g. a core class).
          ClassInfo classInThisCpool = pcdm.getClassInfoForName(verCode, cpoolRefsToMethodClasses[i]);
          if (classInThisCpool == null) continue;  // Class not in project
          if (classInThisCpool.isSubclassOf(methodDefClassInfo.name, false)) {
            ...
            if (! redeclared) return true;
	  } else if (methodDefClassInfo.isInterface() && classInThisCpool.implementsIntfOrSubintfDirectly(methodDefClassName)) return true;
        }
      }
    }    
    return false;
  ...
  public int hasMethodThrowingException(ClassInfo excClassInfo, int startMethodIdx) {
    if (checkedExceptions == null) return -1;
    if (startMethodIdx >= checkedExceptions.length) return -1;
    String excName = excClassInfo.name;
    for (int i = startMethodIdx; i < checkedExceptions.length; i++) {
      if (checkedExceptions[i] == null) continue;
      String[] exc = checkedExceptions[i];
      for (int j = 0; j < exc.length; j++) {
	if (exc[j] == excName) return i;
      }
    }
    return -1;
  }
  ...
  /** For debugging. */
  public String toString() { return name + (verCode == VER_OLD ? " OLD" : " NEW"); }
}
