...
public class ProjectDatabaseWriter extends BinaryFileWriter {
  ...
  private void writeClassInfo(ClassInfo ci) {
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) writeStringRef(cpoolRefsToClasses[i]);
      boolean isRefClassArray[] = ci.isRefClassArray;
      for (i = 0; i < len; i++) buf[curBufPos++] = (isRefClassArray[i] ? (byte)1 : (byte)0);
    }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) writeStringRef(cpoolRefsToFieldClasses[i]);
      String cpoolRefsToFieldNames[] = ci.cpoolRefsToFieldNames;
      for (i = 0; i < len; i++) writeStringRef(cpoolRefsToFieldNames[i]);
      String cpoolRefsToFieldSignatures[] = ci.cpoolRefsToFieldSignatures;
      for (i = 0; i < len; i++) writeStringRef(cpoolRefsToFieldSignatures[i]);
    }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) writeStringRef(cpoolRefsToMethodClasses[i]);
      String cpoolRefsToMethodNames[] = ci.cpoolRefsToMethodNames;
      for (i = 0; i < len; i++) writeStringRef(cpoolRefsToMethodNames[i]);
      String cpoolRefsToMethodSignatures[] = ci.cpoolRefsToMethodSignatures;
      for (i = 0; i < len; i++) writeStringRef(cpoolRefsToMethodSignatures[i]);
    }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) writeStringRef(interfaces[i]);
    }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) writeStringRef(fieldNames[i]);
      String fieldSignatures[] = ci.fieldSignatures;
      for (i = 0; i < len; i++) writeStringRef(fieldSignatures[i]);
      char fieldAccessFlags[] = ci.fieldAccessFlags;
      for (i = 0; i < len; i++) writeChar(fieldAccessFlags[i]);
    }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) writeStringRef(methodNames[i]);
      String methodSignatures[] = ci.methodSignatures;
      for (i = 0; i < len; i++) writeStringRef(methodSignatures[i]);
      char methodAccessFlags[] = ci.methodAccessFlags;
      for (i = 0; i < len; i++) writeChar(methodAccessFlags[i]);
    }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) {
        ...
        writeChar(lenl);
        if (lenl > 0) {
          for (j = 0; j < lenl; j++) writeStringRef(checkedExceptions[i][j]);
        }
      }
    }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) writeStringRef(nestedClasses[i]);
    }
  }
  ...
  private void writeString(String s) {
    ...
    if (curStringBufPos + len > curStringBufWatermark) {
      ...
      if (len >= stringBufInc) stringBufInc = (stringBufInc + len) * 2;
      else stringBufInc = (stringBufInc * 5 ) / 4;  // Still increase a little - observations show that otherwise we usually get here 20 more times
  ...
  static class StringHashTable {
    ...
    final int get(String key) {
      ...
      while (keys[pos] != key &&  keys[pos] != null) pos = (pos + 3) % size; // Relies on the fact that size % 3 != 0
      if (key == keys[pos]) return values[pos];
      else return -1;
    }
    
    final void add(String key, int value) {
      if (nOfElements > watermark) rehash();

      int pos = (key.hashCode() & 0x7FFFFFFF) % size;
      while (keys[pos] != null) pos = (pos + 3) % size;  // Relies on the fact that size % 3 != 0
      keys[pos] = key;
      values[pos] = value;
      nOfElements++;
    }
    ...
    private final int makeLikePrimeNumber(int no) {
      ...
      do {
        no += 2;
        prime = (no % 3 != 0 && no % 5 != 0 && no % 7 != 0 && no % 11 != 0 &&
