...
public class CompatibilityChecker {
  ...
  public void checkDeletedClass(PCDEntry entry) {
    ...
    String packageToLookIn = oldClassInfo.isPublic() ? null : oldClassInfo.packageName;
  ...
  private void checkAccessFlags() {
    ...
    if (oldClassFlags == newClassFlags) return;
    ...
    if (Modifier.isPublic(newClassFlags)) return; 
    ...
    if (Modifier.isProtected(newClassFlags)) {
      ...
      if (! Modifier.isPrivate(oldClassFlags)) versionsCompatible = false;
      else return;  // private -> private, nothing more to check
      if (Modifier.isPublic(oldClassFlags)) rf.findReferencingClasses1(oldClassInfo);
      else if (Modifier.isProtected(oldClassFlags)) rf.findThisPackageOrSubReferencingClasses1(oldClassInfo);
      else rf.findThisPackageReferencingClasses1(oldClassInfo);
  ...
  private void checkSuperclasses() {
    ...
    for (int i = 0; i <= oldNamesSizeMinusOne; i++) {
      ...
      if (! newSuperNames.contains(oldSuperName)) {
        versionsCompatible = false;
	ClassInfo missingSuperClass = pcdm.getClassInfoForName(ClassInfo.VER_OLD, oldSuperName);
	if (missingSuperClass == null) { // This class is not in project
	  missingSuperClass = ClassPath.getClassInfoForName(oldSuperName, pcdm);
	  if (missingSuperClass == null) missingSuperClass = new ClassInfo(oldSuperName, pcdm);
	}
    ...
    if (oldClassInfo.isInterface() || oldSuperNames.size() == 0) return;
    if (! (oldSuperNames.contains("java/lang/RuntimeException") || oldSuperNames.contains("java/lang/Error"))) return;
    if (! (newSuperNames.contains("java/lang/RuntimeException") || newSuperNames.contains("java/lang/Error"))) {
      if (! newSuperNames.contains("java/lang/Throwable")) return;
  ...
  private void checkImplementedInterfaces() {
    ...
    for (Iterator oldIntfIter = oldIntfNames.iterator(); oldIntfIter.hasNext(); ) {
      ...
      if (! newIntfNames.contains(oldIntfName)) {
        versionsCompatible = false;
	ClassInfo missingSuperInterface = pcdm.getClassInfoForName(ClassInfo.VER_OLD, oldIntfName);
	if (missingSuperInterface == null) { // This class is not in project
	  missingSuperInterface = ClassPath.getClassInfoForName(oldIntfName, pcdm);
	  if (missingSuperInterface == null) missingSuperInterface = new ClassInfo(oldIntfName, pcdm);
	}
  ...
  private void checkFields() {
    ...
    for (i = 0; i < oFLen; i++) {
      oFMod = oFFlags[i];
      if (Modifier.isPrivate(oFMod)) continue;  // Changes to private fields don't affect compatibility
      oFName = oFNames[i]; 
      ...
      k = i < nFLen ? i : endIdx;
      for (j = 0; j < nFLen; j++) {
        if (oFName == nFNames[k] && oFSig == nFSignatures[k]) { found = true; break; }
        if (k < endIdx) k++;
        else k = 0;
      }
	...
	if (publicConstantChanged) return;
    ...
    if (nonMatchingNewFields > 0) { // There are some fields declared in the new version which don't exist in the old one
      ...
      for (i = 0; i < nFLen; i++) {
        ...
        boolean found = false;
        for (j = 0; j < oFLen; j++) {
          if (nFName == oFNames[j]) { found = true; break; }
        }
        if (found) continue;  // nFName is not an added field
            
        String superName = oldClassInfo.superName;
        ClassInfo superInfo;
        while (superName != null) {
          superInfo = pcdm.getClassInfoForName(ClassInfo.VER_OLD, superName);
  ...
  private void checkFieldModifiers(int oFMod, int nFMod, int oldFieldIdx, int newFieldIdx) {
    if (oFMod == nFMod) {
      if (Modifier.isFinal(oFMod) &&
        ...
        if (Modifier.isPublic(oFMod)) publicConstantChanged = true;  // Means we will have to recompile ALL project classes
        return;
      }
    }
    ...
    if (Modifier.isStatic(oFMod) && Modifier.isFinal(oFMod) &&  // oFMod is known to be non-private
      ...
      versionsCompatible = false;
      rf.findAllProjectClasses(oldClassInfo, oldFieldIdx);
      if (Modifier.isPublic(oFMod)) publicConstantChanged = true;
      ...
      versionsCompatible = false;
      if (Modifier.isPublic(oFMod)) rf.findDiffPackageReferencingClassesForField(oldClassInfo, oldFieldIdx);
  ...
  private void checkForFinalFields() {
    char oFFlags[] = oldClassInfo.fieldAccessFlags;
    int oFLen = oldClassInfo.fieldNames != null ? oldClassInfo.fieldNames.length : 0;
    int oFMod;
  ...
  private void checkMethodsAndConstructors() {
    ...
    for (i = 0; i < oMLen; i++) {
      oMMod = oMFlags[i];
      if (Modifier.isPrivate(oMMod)) continue;  // Changes to private methods don't affect compatibility
      oMName = oMNames[i]; 
      ...
      k = i < nMLen ? i : endIdx;
      for (j = 0; j < nMLen; j++) {
        if (oMName == nMNames[k] && oMSig == nMSignatures[k]) { found = true; break; }
        if (k < endIdx) k++;
        else k = 0;
      }
      ...
      if (found) {
        ...
        nMMod = nMFlags[k];
        if (oMMod != nMMod) checkMethodModifiers(oMMod, nMMod, i);
        ...
        if (newClassInfo.checkedExceptions != null && newClassInfo.checkedExceptions[k] != null) {
          if (oldClassInfo.checkedExceptions == null) {
            ...
            String oldExceptions[] = oldClassInfo.checkedExceptions[i];
            String newExceptions[] = newClassInfo.checkedExceptions[k];
            for (int ei = 0; ei < newExceptions.length; ei++) {
              String newEx = newExceptions[ei];
              found = false;
              for (int ej = 0; ej < oldExceptions.length; ej++) {
                if (newEx == oldExceptions[ej]){ found = true; break; }
              }
              if (! found) {
                versionsCompatible = false;
                rf.findReferencingClassesForMethod(oldClassInfo, i);
    ...
    if (nonMatchingNewMethods > 0) {  // There are some methods/constructors declared in the new version which don't exist in the old one
      if (! oldClassInfo.isInterface()) {
        for (i = 0; i < nMLen; i++) {
          nMMod = nMFlags[i];
          if (Modifier.isPrivate(nMMod)) continue;
          ...
          boolean found = false;
          for (j = 0; j < oMLen; j++) {
            if (newMName == oMNames[j] && newMSig == oMSignatures[j]) { found = true; break; }
          }
          if (found) continue;  // nMName is not an added method
	  ...
	  oldClassInfo.findExistingSameNameMethods(newMName, newMName != "<init>", false, new ClassInfo.MethodHandler() {
		...
		rf.findReferencingClassesForMethod(classInfo, methodIdx);
          ...
          if (subclassesDeclareSameNameMethod(oldClassInfo, newMName)) versionsCompatible = false;
	}
	...
	for (i = 0; i < nMLen; i++) {
	  nMMod = nMFlags[i];
	    ...
	    oldClassInfo.findExistingSameNameMethods(newMName, true, true, new ClassInfo.MethodHandler() {
	      void handleMethod(ClassInfo classInfo, int methodIdx) {
	        String otherMSig = classInfo.methodSignatures[methodIdx];
	        if (newMSig != otherMSig && Utils.sameParamNumber(newMSig, otherMSig)) rf.findReferencingClassesForMethod(classInfo, methodIdx);
	      }
	    });
  ...
  private void checkMethodModifiers(int oMMod, int nMMod, int oldMethodIdx) {
    if (Modifier.isPrivate(nMMod)) {
      ...
      versionsCompatible = false;
      if (Modifier.isPublic(oMMod)) rf.findDiffPackageReferencingClassesForMethod(oldClassInfo, oldMethodIdx);
      else rf.findDiffPackageAndSubReferencingClassesForMethod(oldClassInfo, oldMethodIdx);
  ...
  private boolean subclassesDeclareSameNameMethod(ClassInfo oldClassInfo, String methodName) {
    ...
    for (int i = 0; i < directSubclasses.length; i++) {
      ...
      if (subclassesDeclareSameNameMethod(subclass, methodName)) res = true;
    }
    return res;
  }
