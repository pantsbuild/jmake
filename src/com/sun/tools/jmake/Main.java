...
public class Main {
  ...
  {"-h", "-help", "-d", "-pdb", "-C", "-jcpath", "-jcmainclass", "-jcmethod", "-jcexec", "-Xtiming", "-version",
   "-warnlimit", "-failondependentjar", "-nowarnondependentjar", "-classpath", "-projclasspath", "-bootclasspath", "-extdirs"};
  private static final int optArgs[] =
  {0, 0, 1, 1, 0, 1, 1, 1, 1, 0, 0, 1, 0, 0, 1, 1, 1, 1};
  ...
  private static int inOptions(String option) {
    if (option.startsWith(optNames[OPT_JAVAC_OPT])) return OPT_JAVAC_OPT;
    for (int i = 0; i < optNames.length; i++) {
      if (option.equals(optNames[i])) return i;
    }
    return -1;
  }
  ...
  private void processCommandLine(String args[]) {
    ...
    for (int i = 0; i < args.length; i++) argsV.add(args[i]);
    int argsSt = 0;
    String arg;
	...
	if (argN != -1) {
	  if (argsSt+optArgs[argN] > argsV.size()) optRequiresArg("\"" + optNames[argN] + "\"");
	} else bailOut(arg + ERR_IS_INVALID_OPTION);

	switch (argN) {
        case OPT_H : case OPT_HELP : 
	  printUsage();
	  throw new PrivateException(new PublicExceptions.NoActionRequestedException());
	case OPT_D :
	...
	case OPT_STORE :
	  ...
	  if (javacArg.equals("-d") || javacArg.equals("-classpath") || javacArg.equals("-bootclasspath") ||
	      javacArg.equals("-extdirs")) bailOut(javacArg + ERR_SHOULD_BE_EXPLICIT);
          javacAddArgs.add(javacArg);
          break;
        case OPT_JCPATH :
          if (jcExecApp != null) bailOut(ERR_NO_TWO_COMPILER_OPTIONS);
        ...
        case OPT_JCMETHOD :
          ...
          break;
        case OPT_JCEXEC :
          if (jcPath != null || jcMainClass != null || jcMethod != null) bailOut(ERR_NO_TWO_COMPILER_OPTIONS);
	...
	case OPT_FAILONDEPJAR :
	  if (noWarnOnDependentJar) bailOut("it is not allowed to use -nowarnondependentjar and -failondependentjar together");
	  failOnDependentJar = true;
	  break;
	case OPT_NOWARNONDEPJAR :
	  if (failOnDependentJar) bailOut("it is not allowed to use -failondependentjar and -nowarnondependentjar together");
	...
	default :
	  ...
	  if (! arg.endsWith(".java")) bailOut(arg + ERR_IS_INVALID_OPTION);
  ...
  private void loadCmdFile(String name, ArrayList argsV) {
    try {
      ...
      st.quoteChar('"');
      st.quoteChar('\'');
      while (st.nextToken() != st.TT_EOF) argsV.add(st.sval);
      r.close();
    } catch (IOException e) {
  ...
  public void mainProgrammatic(String args[]) throws
	...
	if (projectJars != null) {
	  for (int i = 0; i < projectJars.length; i++) allProjectJavaFileNamesList.add(projectJars[i]);
	}
        allProjectJavaFileNames = (String[]) allProjectJavaFileNamesList.toArray(new String[allProjectJavaFileNamesList.size()]);
      } else {
	String[] projectJars = ClassPath.getProjectJars();
	if (projectJars != null) {
	  String newNames[] = new String[allProjectJavaFileNames.length + projectJars.length];
  ...
  private static void printUsage() {
    ...
    Utils.printInfoMessage("  -jcmetod <method>     specify the method to call in the Java compiler class");
