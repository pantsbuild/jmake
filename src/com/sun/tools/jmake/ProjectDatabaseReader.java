...
public class ProjectDatabaseReader extends BinaryFileReader {
  ...
  private void readPreamble() {
    if (buf.length < Utils.magicLength + 8) pdbCorruptedException("file too short");
    
    for (int i = 0; i < Utils.magicLength; i++) {
      if (buf[i] != Utils.MAGIC[i]) pdbCorruptedException("wrong project database header");
    }
      
    curBufPos += Utils.magicLength;
    pdbFormat = nextInt();
    if (pdbFormat != Utils.PDB_FORMAT_CODE_OLD && pdbFormat != Utils.PDB_FORMAT_CODE_LATEST) pdbCorruptedException("wrong version number");
    
    int pdbSize = nextInt();
    if (buf.length != Utils.MAGIC.length + 8 + pdbSize) pdbCorruptedException("file size does not match stored value");
  ...
  private void readStringTable() {
    ...
    for (int i = 0; i < size; i++) stringTable[i] = nextString();
  }
  ...
  private ClassInfo readClassInfo() {
    ...
    if (pdbFormat >= Utils.PDB_FORMAT_CODE_133) res.javacTargetRelease = nextInt();
    else res.javacTargetRelease = Utils.JAVAC_TARGET_RELEASE_OLDEST;
    
    len = nextChar();
    if (len > 0) {
      String cpoolRefsToClasses[] = new String[len];
      for (i = 0; i < len; i++) cpoolRefsToClasses[i] = nextStringRef();
      res.cpoolRefsToClasses = cpoolRefsToClasses;
      boolean isRefClassArray[] = new boolean[len];
      for (i = 0; i < len; i++) isRefClassArray[i] = (buf[curBufPos++] != 0);
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) interfaces[i] = nextStringRef();
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) fieldNames[i] = nextStringRef();
      res.fieldNames = fieldNames;
      String fieldSignatures[] = new String[len];
      for (i = 0; i < len; i++) fieldSignatures[i] = nextStringRef();
      res.fieldSignatures = fieldSignatures;
      char fieldAccessFlags[] = new char[len];
      for (i = 0; i < len; i++) fieldAccessFlags[i] = nextChar();
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) {
        ...
        switch (code) {
          case 1 : primitiveConstantInitValues[i] = nextStringRef(); break;
          case 2 : primitiveConstantInitValues[i] = new Integer(nextInt()); break;
          case 3 : primitiveConstantInitValues[i] = new Long(nextLong()); break;
          case 4 : primitiveConstantInitValues[i] = new Float(nextFloat()); break;
          case 5 : primitiveConstantInitValues[i] = new Double(nextDouble()); break;
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (len1 > 0) {
          ...
          for (j = 0; j < len1; j++) checkedExceptions[i][j] = nextStringRef();
        }
      }
    ...
    if (len > 0) {
      ...
      for (i = 0; i < len; i++) nestedClasses[i] = nextStringRef();
  ...
  private String nextString() {
    int length = nextChar();
    if (buf.length < curBufPos + length) pdbCorruptedException("data error");
